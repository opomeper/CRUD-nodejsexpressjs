...
router.put("/:email", (req, res) => {
  ...
  if (filtered_users.length > 0) {
    ...
    */
    ...
    let firstName = req.query.fisrtName;
    if (firstName) {
...
router.delete("/:email", (req, res) => {
  // Extract email parameter and filter users to find matching email
  const email =req.params.email;
  users = users.filter((user) => user.email !== email);
  // Send success message indicating the user has been deleted
